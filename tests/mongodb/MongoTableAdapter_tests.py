...
def adapter_create_kwargs_works() -> None:
    ...
    with connect(conn_str) as connection:
        ...
        pk = adapter.create(id='kwargs-test-1', value='from_kwargs')
        assert pk is not None
        assert pk.get('id') == 'kwargs-test-1'

        result = adapter.read(id='kwargs-test-1')
        assert result is not None
...
def adapter_query_with_where_works() -> None:
    ...
    with connect(conn_str) as connection:
        ...
        match_id = 'qwhere-match'
        no_match_id = 'qwhere-nomatch'
