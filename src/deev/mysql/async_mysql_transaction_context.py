...
class AsyncMySQLTransactionContext(AsyncDbTransactionContext):
    ...
    async def execute(self, sql: str, parameters: DbParameters | None = None, raw: bool = False) -> AsyncMySQLProxyCursor:
        if not raw:
            modified = self.__preprocess_sql(sql)
            if modified is None:
                from ..common.noop_cursor import NoopCursor
                return cast(AsyncMySQLProxyCursor, NoopCursor())  # type: ignore[return-value]
            sql = modified
