...
class AsyncMSSQLTransactionContext(AsyncDbTransactionContext):
    """
    Async shim that delegates to ``MSSQLTransactionContext``.
    ...
    __inner_ctx: MSSQLTransactionContext | None
    __owns_context: bool

    def __init__(self, context: AsyncDbContext, *, owns_context: bool | None = None) -> None:
        self.__owns_context = owns_context is True
        self.__is_deev_context = isinstance(context, (AsyncMSSQLProxyConnection, AsyncMSSQLTransactionContext))
        self.__context = context if self.__is_deev_context else AsyncMSSQLProxyConnection(context)  # type: ignore[arg-type]
    ...
    def __get_inner_ctx(self) -> MSSQLTransactionContext:
        """Return the underlying sync transaction context, acquiring it if necessary."""
        if self.__inner_ctx is None:
            self.__inner_ctx = MSSQLTransactionContext(
                context=cast(AsyncMSSQLProxyConnection, self.__context).mssql_connection,
                owns_context=self.__owns_context
            )
        return self.__inner_ctx

    async def __aenter__(self) -> Self:
        innser_ctx = self.__get_inner_ctx()
        innser_ctx.__enter__()
        return self
    ...
    async def cursor(self) -> Any:
        ...
        raw_cursor = sync_cursor._MSSQLProxyCursor__cursor  # type: ignore[attr-defined]
        return AsyncMSSQLProxyCursor(raw_cursor)
...
__all__ = ['AsyncMSSQLTransactionContext']
